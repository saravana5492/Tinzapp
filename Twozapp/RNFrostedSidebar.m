...
//    if (inset != 40.0) {
//     self.imageView.frame = CGRectMake(10, inset-30, 40, 40);
//     //self.imageView.layer.cornerRadius = 20.0f;
//        
//    }
//    else
//    {
    self.imageView.frame = CGRectMake(10, inset-20, 40, 40);
   // self.imageView.layer.cornerRadius = 20.0f;
      //  self.label.frame = CGRectMake(55, inset - 15, inset+30, 30);
 //   }
    self.label.frame = CGRectMake(65, inset - 25, 150, 50);
    self.countLabel.frame = CGRectMake(180, inset - 10, 30, 21);
   // self.imageView.center = CGPointMake(10, 10);
    ...
    self.countLabel.textColor = [UIColor colorWithRed:200.0/255.0 green:22.0/255.0 blue:55.0/255.0 alpha:1.0];
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors textLabels:(NSArray *)textLabels {
    if (self = [super init]) {
        ...
        _itemSize = CGSizeMake(_width/2, 80);
        ...
        if (textLabels) {
            NSAssert([textLabels count] == [images count], @"TextLabels count must match images count. If you want a blank border, use [UIColor clearColor].");
        }
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
            ...
            if (idx == 1) {
                view.imageView.layer.cornerRadius = 20.0f;
                [view.imageView.layer setMasksToBounds:YES];
            }
            view.countLabel.hidden = YES;
            if (idx == 3) {
                view.countLabel.hidden = NO;
                int totalCount = 0;
                for (id key in [[self appDelegate] chatCounts]) {
                    totalCount += [[[[self appDelegate] chatCounts] objectForKey:key] intValue];
                }
                ...
                {
                    view.countLabel.text = [NSString stringWithFormat:@"%i",totalCount];
                    view.countLabel.hidden = NO;
                }
            }
          //  RNCalloutTextItemView *textView = [[RNCalloutTextItemView alloc] init];
            
            view.label.text = [_textLabels objectAtIndex:idx];
            view.backgroundColor = [colors objectAtIndex:idx];
          //  [_contentView addSubview:textView];
           
            [_contentView addSubview:view];
            
            [_itemViews addObject:view];
           // [_itemViews addObject:textView];
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices {
    return [self initWithImages:images selectedIndices:selectedIndices borderColors:nil textLabels:nil];
}

- (instancetype)initWithImages:(NSArray *)images {
    return [self initWithImages:images selectedIndices:nil borderColors:nil textLabels:nil];
}
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    self.view.frame =  CGRectMake(controller.view.bounds.origin.x, 64, controller.view.bounds.size.width, controller.view.bounds.size.height - 64);
    ...
    [self.itemViews enumerateObjectsUsingBlock:^(RNCalloutItemView *view, NSUInteger idx, BOOL *stop) {
        ...
        if (idx%2 == 0) {
            view.originalBackgroundColor = [UIColor colorWithRed:146.0/255.0 green:46.0/255.0 blue:86.0/255.0 alpha:1.0];
            view.backgroundColor = [UIColor colorWithRed:146.0/255.0 green:46.0/255.0 blue:86.0/255.0 alpha:1.0];
        }
        else
        {
            view.originalBackgroundColor = [UIColor colorWithRed:206.0/255.0 green:27.0/255.0 blue:59.0/255.0 alpha:1.0];
            view.backgroundColor = [UIColor colorWithRed:206.0/255.0 green:27.0/255.0 blue:59.0/255.0 alpha:1.0];
        }
...
- (void)layoutItems {
    ...
    [self.itemViews enumerateObjectsUsingBlock:^(RNCalloutItemView *view, NSUInteger idx, BOOL *stop) {
        ...
        CGRect frame;
//        if (idx == 0) {
//            frame = CGRectMake(0, 1*idx+height*idx+1, self.width, height + height/2);
//        }
//        else
//        {
            frame = CGRectMake(0, 1*idx+height*idx+1, self.width, height);
//        }
    ...
    self.contentView.contentSize = CGSizeMake(0, items *height+8 + height/2);
}
